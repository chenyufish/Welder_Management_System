...
import com.fishman.management_backend.mapper.TeamMapper;
...
import com.fishman.management_backend.model.enums.TeamStatusEnum;
import com.fishman.management_backend.model.request.*;
import com.fishman.management_backend.model.vo.TeamUserVO;
import com.fishman.management_backend.model.vo.UserVO;
import com.fishman.management_backend.properties.FishmanProperties;
import com.fishman.management_backend.service.FollowService;
import com.fishman.management_backend.service.TeamService;
import com.fishman.management_backend.service.UserService;
import com.fishman.management_backend.service.UserTeamService;
import com.fishman.management_backend.utils.FileUtils;
...
import static com.fishman.management_backend.contant.SystemConstants.PAGE_SIZE;
import static com.fishman.management_backend.contant.TeamConstants.*;
...
public class TeamServiceImpl extends ServiceImpl<TeamMapper, Team> implements TeamService {
     ...
     */
    @Override
    public Page<TeamUserVO> listTeams(long currentPage, TeamQueryRequest teamQuery, boolean isAdmin) {
    ...
    public TeamUserVO getTeam(Long teamId, Long userId) {
        ...
        TeamUserVO.setHasJoin(userJoin > 0);
        User leader = userService.getById(team.getUserId());
        TeamUserVO.setLeaderName(leader.getUsername());
    ...
    public Page<TeamUserVO> listTeamByCondition(long currentPage, LambdaQueryWrapper<Team> teamLambdaQueryWrapper) {
        ...
        // 关联查询创建人的用户信息
        BeanUtils.copyProperties(teamPage, TeamUserVOPage, "records");
    ...
    @Override
    public Page<TeamUserVO> listMyCreate(long currentPage, Long userId) {
        LambdaQueryWrapper<Team> teamLambdaQueryWrapper = new LambdaQueryWrapper<>();
        teamLambdaQueryWrapper.eq(Team::getUserId, userId);
        Page<Team> teamPage = this.page(new Page<>(currentPage, PAGE_SIZE), teamLambdaQueryWrapper);
        List<TeamUserVO> TeamUserVOList = teamPage.getRecords()
                .stream().map((team) -> this.getTeam(team.getId(), userId))
                .collect(Collectors.toList());
        Page<TeamUserVO> TeamUserVOPage = new Page<>();
        BeanUtils.copyProperties(teamPage, TeamUserVOPage);
        TeamUserVOPage.setRecords(TeamUserVOList);
        return TeamUserVOPage;
    }

    @Override
    public Page<TeamUserVO> getJoinedUserAvatar(Page<TeamUserVO> TeamUserVOPage) {
        TeamUserVOPage.getRecords().forEach((item) -> {
